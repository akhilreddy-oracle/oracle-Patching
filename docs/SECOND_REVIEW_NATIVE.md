...
| Recovery's live identity parser selected the last nonempty SQL output row, discarding preceding conflicting rows or diagnostics. This made its duplicate-evidence rejection weaker than the patch adapters' gate. | Recovery analysis and pre-downtime execution now require exactly one normalized nonempty row with ten fields, including rejection of trailing delimiters. SQL*Plus exits on operating-system errors as well as SQL errors. Duplicate, conflicting, diagnostic-prefixed, truncated, extra-field and empty output regressions assert blocked admission, unchanged running services, no shutdown/listener-stop commands and no backup directory. Blank surrounding lines and whitespace remain accepted. |
| Five database adapters selected the last matching keyed SQL row; some SQL registry probes also discarded stderr. Conflicting health/action/status records or SQL diagnostics could therefore accompany an accepted success. | Shared scalar parsing consumes the entire captured probe, rejects duplicate/missing/empty required values and Oracle/SQL*Plus diagnostics, and preserves legitimate banners, whitespace and unrelated multi-row component results. Registry stderr is captured; OJVM/home-switch registry probes also request exit on operating-system errors. The standalone dictionary check explicitly rejects query failure even inside a shell conditional. Actual health, APPLY/ROLLBACK registry and dictionary functions have malformed-output regressions. |
