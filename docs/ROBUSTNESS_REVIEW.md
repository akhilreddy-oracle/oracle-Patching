...
This review covers the current application and native execution code, with fixes
and isolated regressions. **Final validation of the combined working tree is
pending.** Passing an earlier revision or an individual suite does not establish
...
| 8. Automated release validation | Pull-request workflow, native/controller/frontend checks, isolated Chromium tests, failure drills and exact-source receipts. The UI separates fixture, live-lab and production evidence. | Finish the combined-tree Mac/Linux and browser gates, retain their receipts, then collect independently reviewed live-lab evidence and production approval for the exact runtime/configuration. |
...
These are targeted results reported during the audit, not a summed full-release
test count. The final release runner must revalidate the combined snapshot.

| Check | Latest targeted result | Evidence boundary |
| --- | --- | --- |
| Frontend Node suites | 91 tests passed | DOM fixtures and application behavior; no Oracle operations. |
| Chromium browser cases | 31 cases passed across targeted runs | An initially missing mock route was corrected and the affected workflow suite rerun. This is not a new combined-tree browser receipt. |
...
| Combined release check / Mac / Linux / browser | **Pending** | Root release validation will record final commands, source digest, results and retained bundle location. |

`git diff --check` passed after the completed fixes. The final gate must include
all newly registered regression suites and the completed Data Guard integration
changes.

## Remaining evidence, not inferred completion

- Retain exact-source results for the completed combined release gates.
