...
mkdir -p "$TMP/artifact/etc/config" "$TMP/artifact/files"
...
mkdir -p "$TMP/modern/etc/config" "$TMP/modern/files"
...
printf 'payload\n' >"$TMP/modern/files/placeholder.bin"
...
mkdir -p "$TMP/thin/etc/config"
printf '<patch patchID="11111111"><os_platforms><platform id="226" name="Linux x86-64"/></os_platforms></patch>\n' >"$TMP/thin/etc/config/inventory.xml"
printf '<patch patchID="11111111"/>\n' >"$TMP/thin/etc/config/actions.xml"
if "$ROOT/bin/opu-artifact-inspect" --artifact "$TMP/thin" --output "$TMP/thin.json" >/dev/null 2>&1; then
  echo 'metadata-only artifact without files/ was accepted' >&2
  exit 1
fi
jq -e '.artifact.status == "blocked" and (.artifact.reason | test("missing files/"))' "$TMP/thin.json" >/dev/null
mkdir -p "$TMP/no-platform/etc/config" "$TMP/no-platform/files"
printf '<patch patchID="99999999"><os_platforms/></patch>\n' >"$TMP/no-platform/etc/config/inventory.xml"
printf 'payload\n' >"$TMP/no-platform/files/placeholder.bin"
