...
APPLY_EXECUTOR="$ROOT/bin/opu-grid-node-patch"
ROLLBACK_EXECUTOR="$ROOT/bin/opu-grid-node-rollback"
PLAN_STATE="$TMP/plan-state"
APPLY_EXECUTION_STATE="$TMP/apply-execution-state"
...
printf '%s\n' \
  ...
  'opatch rollback -id 39034528' >"$PATCH_DIR/README.txt"
...
create_apply_plan() {
  plan create --plan-id grid-apply-success --requester patch-admin \
...
execute_apply_task() {
  local task_id=$1 node_name=$2
  OPU_PLAN_STATE_DIR="$PLAN_STATE" \
  OPU_GRID_NODE_STATE_DIR="$APPLY_EXECUTION_STATE" \
  ...
  OPU_TEST_GRID_RUNTIME="$RUNTIME_ROOT" \
    bash "$APPLY_EXECUTOR" execute --plan-id grid-apply-success --task-id "$task_id" --actor grid-worker --lease-seconds 30
}

run_apply_expected() {
  local expected_stage=$1 expected_node=$2 task_json task_id
  task_json=$(plan next --plan-id grid-apply-success)
  task_id=$(jq -r '.task_id' <<<"$task_json")
  [ "$(jq -r '.stage' <<<"$task_json")" = "$expected_stage" ]
  [ "$(jq -r '.node' <<<"$task_json")" = "$expected_node" ]
  execute_apply_task "$task_id" "$expected_node"
}
...
execute_rollback_task() {
  ...
  OPU_PLAN_STATE_DIR="$PLAN_STATE" \
  OPU_GRID_NODE_STATE_DIR="$ROLLBACK_EXECUTION_STATE" \
  OPU_GRID_NODE_TEST_MODE=1 \
  OPU_GRID_NODE_TEST_HOST="$node_name" \
  OPU_GRID_NODE_TEST_OWNER="$OWNER" \
  OPU_TEST_GRID_RUNTIME="$RUNTIME_ROOT" \
    bash "$ROLLBACK_EXECUTOR" execute --plan-id "$plan_id" --task-id "$task_id" --actor rollback-worker --lease-seconds 30
}
...
# Complete a succeeded Grid apply plan that rollback can derive from.
create_apply_plan
for node_name in node1 node2; do
  for stage_name in grid_precheck grid_rootcrs_prepatch grid_opatch_apply grid_rootadd_rdbms grid_rootcrs_postpatch grid_node_validate; do
    run_apply_expected "$stage_name" "$node_name" >/dev/null
  done
done
run_apply_expected grid_cluster_final_validate node1 >/dev/null
plan status --plan-id grid-apply-success | jq -e '.state == "succeeded"' >/dev/null
[ -f "$RUNTIME_ROOT/patch-node1.state" ] && [ -f "$RUNTIME_ROOT/patch-node2.state" ]

# Source apply worker cannot approve the derived rollback plan.
plan create-rollback --plan-id grid-rollback-sod --requester rollback-admin \
  --source-plan-id grid-apply-success --window-start "$WINDOW_START" --window-end "$WINDOW_END" >/dev/null
plan status --plan-id grid-rollback-sod | jq -e '.source_apply.actors == ["grid-worker"]' >/dev/null
if plan approve --plan-id grid-rollback-sod --actor grid-worker --approval-ticket TEST-GRID-ROLLBACK-SOD >/dev/null 2>&1; then
  echo 'source apply worker was allowed to approve Grid rollback' >&2
  exit 1
fi
plan approve --plan-id grid-rollback-sod --actor rollback-approver --approval-ticket TEST-GRID-ROLLBACK-SOD >/dev/null
if plan authorize --plan-id grid-rollback-sod --actor grid-worker >/dev/null 2>&1; then
  echo 'source apply worker was allowed to authorize Grid rollback' >&2
  exit 1
fi

# A fresh rollback child plan executes both nodes in reverse apply order.
create_rollback_plan grid-rollback-success
jq -e '.procedure.adapter == "grid_rolling_opatch_rollback" and .nodes == ["node2","node1"] and (.source_apply.lineage | length) == 13' \
  "$PLAN_STATE/plans/grid-rollback-success/plan.json" >/dev/null
for node_name in node2 node1; do
  for stage_name in grid_rollback_precheck grid_rootcrs_prepatch grid_opatch_rollback grid_rootcrs_postpatch grid_rollback_node_validate; do
    result=$(run_rollback_expected grid-rollback-success "$stage_name" "$node_name")
    jq -e --arg stage "$stage_name" '
      .collector.name == "oracle.grid.node.rollback.executor" and .intent == "patch_rollback" and
...
run_rollback_expected grid-rollback-success grid_rollback_cluster_final_validate node1 >/dev/null
plan status --plan-id grid-rollback-success | jq -e '.intent == "patch_rollback" and .state == "succeeded"' >/dev/null
[ ! -f "$RUNTIME_ROOT/patch-node1.state" ] && [ ! -f "$RUNTIME_ROOT/patch-node2.state" ]
printf '%s\n' 'Grid local node rollback executor test passed'
