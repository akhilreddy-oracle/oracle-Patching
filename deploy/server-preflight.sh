...
for command in python3 bash jq xmllint tar gzip ssh sha256sum flock systemctl nginx useradd; do
...
print('PYTHON_VENV ' + str(importlib.util.find_spec('venv') is not None))
print('PYTHON_ENSUREPIP ' + str(importlib.util.find_spec('ensurepip') is not None))
PY
fi
